...
let
  ...
  replace' = oldAttrs: if builtins.isFunction replace then replace oldAttrs else replace;
in
overridenAttr.overrideAttrs (
  ...
  (
    ...
    // lib.optionalAttrs (extra ? mesonFlags) { mesonFlags = oldAttrs.mesonFlags ++ extra.mesonFlags; }
